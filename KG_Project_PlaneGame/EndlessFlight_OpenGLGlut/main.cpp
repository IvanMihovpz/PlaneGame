...
void singlestoryHouse(int R,int G,int B){
...
///Rings
    ...
    glPushMatrix();
        ...
        glutSolidTorus(0.5,4,30,30);
    glPopMatrix();

        for(int i=-(EN_SIZE/2)+1;i<(EN_SIZE/2);i+=2){
            for(int j=-(EN_SIZE/2)+1;j<(EN_SIZE/2);j+=2){
                if(story[i+(EN_SIZE/2)+1][j+(EN_SIZE/2)+1]!=0){
                    glPushMatrix();
                        glTranslated(i,0,j);
                        house(story[i+(EN_SIZE/2)+1][j+(EN_SIZE/2)+1],i,j);
                    glPopMatrix();
                }else if(i>=-5&&i<=5){}
                else{
                    story[i+(EN_SIZE/2)+1][j+(EN_SIZE/2)+1]=(rand()%5)+1;
                    glPushMatrix();
                        glTranslated(i,0,j);
                        house(story[i+(EN_SIZE/2)+1][j+(EN_SIZE/2)+1],i,j);
                    glPopMatrix();
...
static void display(void)
{
    ...
    if(START){
        ...
        drawStrokeText("UP: S, DOWN: W, LEFT: A, RIGHT: D, MAIN MENU: M",-8,0.9,0);
        drawStrokeText("SCORE:",5,0,0);
       // int mod1,number=0;
//        while(SCORE){
       //     mod1=;
       //     number=number+10;
        ...
        drawStrokeText("Press F to Start",-1,-1,0);
        drawStrokeText2("Endless Flight",-2,0,0);
    }
